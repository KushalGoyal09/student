...
export default function SalaryManagement() {
  ...
  const [mentors, setMentors] = useState<{
    GroupMentor: (GroupMentor | SeniorMentor)[];
    SeniorMentor: (GroupMentor | SeniorMentor)[];
    Employee: Employee[];
  }>({
    GroupMentor: [],
    SeniorMentor: [],
  ...
  const handleEditSalary = async (
    ...
    const updatedSalaries = salaries[activeTab].map((salary) =>
      salary.userId === userId ? {...salary, totalSalary, bonus, paid} : salary,
    );
  ...
  const calculateTotalSalary = (
    ...
    if (commonSalary.payAccordingToRating) {
      return mentor.overallRating >= 4.5
        ? commonSalary.perAjMore * mentor.studentCount + commonSalary.baseSalary
        : commonSalary.perAjLess * mentor.studentCount +
            commonSalary.baseSalary;
    } else {
      return commonSalary.perAj * mentor.studentCount + commonSalary.baseSalary;
    }
  };

  const totalPaid = salaries[activeTab]
    .filter((salary) => salary.paid)
    .reduce((sum, salary) => sum + salary.totalSalary + salary.bonus, 0);
  const totalToPay = salaries[activeTab]
    .filter((salary) => !salary.paid)
    .reduce((sum, salary) => sum + salary.totalSalary + salary.bonus, 0);

  return (
    <div className="container mx-auto px-4 py-6 sm:p-6 bg-gray-50 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-pcb">Salary Management</h1>
          ...
          <Card className="bg-white shadow-lg rounded-lg overflow-hidden">
            <CardHeader className="bg-pcb text-white p-6">
              <CardTitle className="text-xl">
                <p className="mb-2">
                  Total To Pay:{" "}
                  <span className="text-red-500"> {totalToPay.toFixed(2)}</span>
                </p>
                <p>
                  Total Paid:{" "}
            ...
            <CardContent className="p-6">
              {commonSalary && activeTab !== "Employee" && (
                <div className="mb-6 p-6 border rounded-lg shadow-sm bg-gray-50">
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6">
                    <div className="flex flex-col">
                      <Label htmlFor="basePay" className="mb-2 text-pcb">
                        BASE PAY
                      </Label>
                      <Input
                        type="number"
                        id="basePay"
                        value={commonSalary.baseSalary || ""}
                        onChange={(e) =>
                          setCommonSalary({
                            ...commonSalary,
                    ...
                    {!commonSalary.payAccordingToRating && (
                      <div className="flex flex-col">
                        <Label htmlFor="perAj" className="mb-2 text-pcb">
                          PER AJ
                        </Label>
                        <Input
                          type="number"
                          id="perAj"
                          value={commonSalary.perAj || ""}
                          onChange={(e) =>
                            setCommonSalary({
                              ...commonSalary,
                              perAj: Number(e.target.value),
                            })
                          }
                          placeholder="Per AJ"
                          className="w-full border-pcb"
                    ...
                    <div className="flex items-center space-x-2 col-span-full">
                      <input
                        type="checkbox"
                        id="payAsPerRating"
                        checked={commonSalary.payAccordingToRating}
                        onChange={(e) =>
                          setCommonSalary({
                            ...commonSalary,
                            payAccordingToRating: e.target.checked,
                          })
                        }
                    ...
                    {commonSalary.payAccordingToRating && (
                      <>
                        <div className="flex flex-col">
                          <Label htmlFor="perAjLess" className="mb-2 text-pcb">
                            PER AJ (rating &lt; 4.5)
                          </Label>
                          <Input
                            type="number"
                            id="perAjLess"
                            value={commonSalary.perAjLess || ""}
                            onChange={(e) =>
                              setCommonSalary({
                                ...commonSalary,
                                perAjLess: Number(e.target.value),
                              })
                            }
                        ...
                        <div className="flex flex-col">
                          <Label htmlFor="perAjMore" className="mb-2 text-pcb">
                            PER AJ (rating &gt; 4.5)
                          </Label>
                          <Input
                            type="number"
                            id="perAjMore"
                            value={commonSalary.perAjMore || ""}
                            onChange={(e) =>
                              setCommonSalary({
                                ...commonSalary,
                                perAjMore: Number(e.target.value),
                              })
                            }
                  ...
                  <Table>
                    <TableHeader className="sticky top-0 bg-white z-20">
                      <TableRow className="bg-pcb/10">
                        ...
                        <TableHead className="text-pcb">Total Salary</TableHead>
                        <TableHead className="text-pcb">Bonus</TableHead>
                        <TableHead className="text-pcb">Paid</TableHead>
                        <TableHead className="text-pcb">Actions</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {mentors[activeTab].map((mentor) => {
                        const salary = salaries[activeTab].find(
                        ...
                        return (
                          ...
                          >
                            ...
                            {activeTab === "Employee" && (
                              <TableCell>
                                {(mentor as Employee).phoneNumber}
                              </TableCell>
                            )}
                            <TableCell>
                              <Input
                            ...
                            <TableCell>
                              <Button
                                ...
                                variant={salary.paid ? "default" : "outline"}
                                className={
                                  salary.paid
                            ...
                            <TableCell>
                              {isEditing ? (
                                ...
                                <Button
                                  onClick={() => setEditingId(mentor.id)}
                                  className="bg-pcb hover:bg-pcb/90 text-white w-full sm:w-auto"
                                >
                                  Edit
                                </Button>
